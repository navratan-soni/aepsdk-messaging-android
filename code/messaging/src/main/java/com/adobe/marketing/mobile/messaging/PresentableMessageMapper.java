...
class PresentableMessageMapper {
     ...
     * @param propositionItem {@link PropositionItem} instance containing item data of type {@link
     *     InAppSchemaData}
    ...
    Message createMessage(
        ...
        final InternalMessage internalMessage =
                new InternalMessage(messagingExtension, propositionItem, assetMap, propositionInfo);
    ...
    static class InternalMessage implements Message {
         ...
         * @param propositionItem {@link PropositionItem} instance containing item data of type
         *     {@link InAppSchemaData}
        ...
        private InternalMessage(
            ...
            if (propositionItem == null) {
                Log.debug(
                        MessagingConstants.LOG_TAG,
                        SELF_TAG,
                        "Unable to create an in-app message, PropositionItem is null.");
                throw new MessageRequiredFieldMissingException(
                        "Required field: \"PropositionItem\" is null.");
            }
            ...
            if (SchemaType.INAPP != schemaType) {
                Log.debug(
                        MessagingConstants.LOG_TAG,
                        SELF_TAG,
                        "Required field \"schema\" is (%s) should be of type (%s).",
                        schemaType,
            ...
            if (inAppSchemaData == null) {
                Log.debug(
                        MessagingConstants.LOG_TAG,
                        SELF_TAG,
                        "In-app message proposition item data is null or empty.");
                throw new MessageRequiredFieldMissingException(
                        "Required field: in-app message proposition item \"data\" is null or"
                                + " empty.");
            }
            ...
            try {
                ...
                if (StringUtils.isNullOrEmpty(html)) {
                    Log.warning(
                            MessagingConstants.LOG_TAG,
                            SELF_TAG,
                            "Unable to create an in-app message, the html payload is null or"
                                    + " empty.");
                    throw new MessageRequiredFieldMissingException(
                            "Required field: in-app message \"content\" is null or empty.");
                }

                final InAppMessageSettings settings =
                        InAppMessageSettingsFromMap(
                                inAppSchemaData.getMobileParameters(), html, assetMap);
                aepMessage =
                        uiService.create(
                ...
                Log.warning(
                        MessagingConstants.LOG_TAG,
                        SELF_TAG,
                        "Unable to create an in-app message, in-app message content is not of type"
                                + " String.");
                throw new MessageRequiredFieldMissingException(
